...
ARG soft="python3 cmake make clang-${CLANG_VERSION} libomp-${CLANG_VERSION}-dev clang++-${CLANG_VERSION} texinfo build-essential fakeroot devscripts wget"
RUN echo ${CLANG_VERSION}
RUN apt-get update 
RUN apt-get install -y ${deps} --no-install-recommends --force-yes
RUN curl https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add -
RUN add-apt-repository "deb http://apt.llvm.org/buster/ llvm-toolchain-buster-${CLANG_VERSION} main"
RUN apt-get update
